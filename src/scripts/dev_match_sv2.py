...
def main() -> None:
    ...
    sv2 = parse_sv(xlsx_path=xlsx, judet="B", sv=2)
    print("SV:", sv2["sv"])
    print("Rules streets:", [r.get("street") for r in sv2.get("rules", [])])
    ...
    print(f"Matched addresses for SV{sv2['sv']}: {len(matched)}")
    ...
    out_path = out_dir / "sv2_addresses.gpkg"
    matched.to_file(out_path, layer="sv2_addresses", driver="GPKG")
