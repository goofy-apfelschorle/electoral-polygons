# src/scripts/dev_scrape_storia_bucharest.py
...
BASE_URL = "https://www.storia.ro/ro/rezultate/inchiriere/apartament/bucuresti"

# Set a high cap; the script will STOP automatically when it reaches the end / repeats.
MAX_PAGES = 1000

# If you truly want ~300 pages, this range is a reasonable compromise.
SLEEP_RANGE = (0.8, 2.0)

# Save a partial checkpoint every N pages (so you don't lose progress)
CHECKPOINT_EVERY = 25
...
def main():
    out_dir = Path("rezultate_chirii") / "raw"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "storia_bucuresti_apartamente_inchiriere.csv"
    partial_path = out_dir / "storia_bucuresti_apartamente_inchiriere__partial.csv"
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        # End condition 1: no cards found
        if len(listings) == 0:
            print("No listings found — stopping.")
            break

        # End condition 2: repeated content (no new URLs)
        ...
        # Keep the page data (we dedupe at the end)
